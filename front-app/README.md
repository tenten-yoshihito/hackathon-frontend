...
1.  **Page (`Home.tsx`)**
    *   ホーム画面がマウントされる、またはタブが「すべて」に切り替わる。
    *   `useEffect` 内でデータ取得関数 (`fetchData`) を呼び出す。
    *   ローディング状態 (`loadingAll`) を `true` に設定。

2.  **API Client (`lib/api/item_list.ts`)**
    *   バックエンド (`GET /items`) へHTTPリクエストを送信。
    *   ページネーション用のパラメータ (`limit`, `offset`) を付与。
    *   レスポンス (商品一覧) を受け取り、JSONをパースして返す。

3.  **Page (`Home.tsx`)**
    *   データを受け取り、状態 (`allItems`) を更新。
        *   初期表示時 (`offset=0`) はリストを上書き。
        *   追加読み込み時 (`offset>0`) は既存リストの末尾に追加。
    *   ローディング状態を `false` に設定。
    *   `ItemGrid` コンポーネントにデータを渡して描画。
